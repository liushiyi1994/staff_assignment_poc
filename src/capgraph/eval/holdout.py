...
class BenchmarkManifestEntry(BaseModel):
    ...
    # v1 treats one issue as a query at its creation time. Benchmark v4 groups issues
    # into work packages and asks the question at the package's start boundary, which
    # is the sprint's recorded start date (docs/benchmark-v4-manifest.md).
    query_time_source: Literal["created_at", "sprint_start"] = "created_at"
    project_key: str
...
def filter_history_as_of(
    ...
    *,
    ...
    for column in (
        ...
        "assigned_at",
        # Unversioned final snapshots carried for the v4 manifest and for audit.
        "sprint_id",
        "story_point",
        "timespent",
        "in_progress_minutes",
    ...
    if "sprint_provenance" in history:
        history["sprint_provenance"] = "redacted_unversioned_final_snapshot"
...
def _stable_score(
    ...
    """Deterministic per-case sort key.

    ``version`` is part of the hash so two manifest versions never draw the same
    sample from overlapping candidates. It defaults to v1's version, which is what
    keeps the frozen v1 manifest reproducible from this code.
    """
    value = f"{version}\0{seed}\0{purpose}\0{project_key}\0{issue_id}"
    ...
    for project_key, indexes in by_project.items():
        indexes.sort(
            key=lambda index: _stable_score(
                seed, project_key, entries[index].issue_id, "sample", version
            )
        )
...
def _assign_splits(
    ...
    *,
    ...
    for project_key, indexes in sorted(by_project.items()):
        indexes.sort(
            key=lambda index: _stable_score(
                seed, project_key, entries[index].issue_id, "split", version
            )
        )
