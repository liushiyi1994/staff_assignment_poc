...
Rankers can be wired in later through :func:`evaluate`; each receives a query context
...
def _query_context(case: BenchmarkManifestEntry) -> BenchmarkQueryContext:
    """Validate selected-case invariants and construct a truth-free ranker input."""
    if case.manifest_version != MANIFEST_VERSION:
        raise ValueError(
            f"manifest version {case.manifest_version!r} does not match {MANIFEST_VERSION!r}"
        )
...
def write_results(results: Sequence[EvaluationResult], path: Path = RESULTS_PATH) -> None:
    ...
    lines = [
        ...
        "Candidate recall | Mean latency (ms) | P95 latency (ms) | Cost (USD) |",
        "|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for result in results:
        lines.append(_summary_row(result.system, result))
    ...
    if results:
        lines.extend(
            [
                ...
                "## Per-project results",
                "",
                "| System / project | N | Hit@1 | Hit@5 | Hit@10 | Recall@5 | Recall@10 | "
                "MRR | Candidate recall | Mean latency (ms) | P95 latency (ms) | Cost (USD) |",
                "|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
            ]
        )
        for result in results:
            for project_key, summary in result.per_project.items():
                lines.append(_summary_row(f"{result.system} / {project_key}", summary))
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")


def _summary_row(label: str, summary: MetricSummary) -> str:
    return (
        ...
        f"{summary.latency_ms_mean:.1f} | {summary.latency_ms_p95:.1f} | "
...
def main() -> None:
    """Validate the manifest without running retrieval, graph, or LLM stages."""
        ...
        json.dumps(
            {
                ...
                "note": "No rankers or LLMs were run; use evaluate() to score wired systems.",
            },
            indent=2,
        )
    )
