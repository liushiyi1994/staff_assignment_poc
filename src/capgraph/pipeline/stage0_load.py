...
CHANGE_LOG_SQL = text(
    ...
    WHERE p.`Project_Key` IN :project_keys
      AND LOWER(TRIM(cl.`Field`)) IN (
          ...
          'project', 'key', 'resolution', 'resolutiondate', 'sprint'
      )
...
# Benchmark v4 groups work into sprints (docs/benchmark-v4-manifest.md). ``JiraID`` is
# the id the sprint change log writes, and it is unique only *within* a project in this
# dump, so every join on it must carry the project key with it.
SPRINTS_SQL = text(
    """
    SELECT
        s.`ID` AS sprint_id,
        ...
        s.`Activated_Date` AS activated_date,
        s.`Complete_Date` AS complete_date
    FROM `Sprint` AS s
    JOIN `Project` AS p
      ON p.`ID` = s.`Project_ID`
    WHERE p.`Project_Key` IN :project_keys
    ORDER BY p.`Project_Key`, s.`ID`
    """
).bindparams(bindparam("project_keys", expanding=True))
...
# A sentence terminator plus any closing quote or bracket, where the next character is
# whitespace. Requiring the whitespace is what keeps "3.24" and "e.g." from reading as
# sentence ends; the text has already been whitespace-collapsed by the time this runs.
SENTENCE_END = re.compile(r"[.!?][\"')\]]*(?=\s)")


def truncate_at_boundary(text_: str, max_chars: int) -> str:
    """Truncate to at most ``max_chars``, at the last sentence end, else a word end.

    Backlog G1. The old cut was a blind character slice that could land mid-word, and a
    clipped clause reads to the extractor as though the sentence said something it did
    not — the same argument `config/settings.yaml` already makes about clipped
    contribution summaries in the re-rank view.

    Deliberately limited. It does **not** address the larger half of G1 — that the first
    ``max_chars`` characters of a Jira description are not necessarily the informative
    ones — because every remedy for that (head-plus-tail, log stripping, relevance
    selection) is a selection decision, and Stage 0 is the provenance layer: if it
    selects or rewrites, "here is the evidence" stops being literally true.

    A sentence end is only taken when it keeps at least
    ``bucketing.min_truncation_keep_fraction`` of the budget, so one enormous opening
    sentence loses a few trailing characters rather than the whole description.
    """
    if max_chars <= 0:
        return ""
    if len(text_) <= max_chars:
        return text_
    # One character past the budget, so a boundary sitting exactly on it is visible.
    window = text_[: max_chars + 1]
    keep_at_least = max_chars * float(settings["bucketing.min_truncation_keep_fraction"])
    ends = [match.end() for match in SENTENCE_END.finditer(window) if match.end() <= max_chars]
    if ends and ends[-1] >= keep_at_least:
        return text_[: ends[-1]]
    head = text_[:max_chars]
    if window[max_chars].isspace():
        return head                          # the budget already ends on a word boundary
    cut = head.rfind(" ")
    return head[:cut] if cut >= keep_at_least else head
...
def strip_markup(text_: str | None, max_chars: int) -> str | None:
    ...
    return truncate_at_boundary(cleaned, max_chars) or None
...
def _optional_float(value: Any) -> float | None:
    """Carry a numeric TAWOS column through without inventing a zero for a null."""
    if _is_missing(value):
        return None
    try:
        return float(value)
        ...
        records.append(
            {
                ...
                "sprint_id": _stable_id(row.sprint_id),
                "sprint_provenance": (
                    "final_snapshot_with_recorded_sprint_change"
                    if field_changes.get("sprint")
                    else "final_snapshot_no_recorded_change"
                ),
                "story_point": _optional_float(row.story_point),
                "timespent": _optional_float(row.timespent),
                "in_progress_minutes": _optional_float(row.in_progress_minutes),
                "total_effort_minutes": _optional_float(row.total_effort_minutes),
            }
        )
...
def _sprint_id_set(value: Any) -> set[int]:
    """Parse one side of a sprint transition: a comma-separated list of Jira ids."""
    if _is_missing(value):
        return set()
    ids: set[int] = set()
...
def _sprint_membership_frame(
    tickets: pd.DataFrame,
    change_rows: pd.DataFrame,
    sprints: pd.DataFrame,
) -> pd.DataFrame:
    """Dated sprint joins from the change log, plus the undated final snapshot.

    The change log records the sprint field as the *whole* set of sprints an issue
    belongs to, so a join is the difference between the two sides of a transition and
    carries the transition's timestamp.  That timestamp is the only thing that makes a
    package's as-of rule enforceable: without it we could not tell work that was planned
    into a sprint before it started from work that was pulled in afterwards.

    Final-snapshot memberships are exported alongside with ``added_at`` null and their
    own provenance, because their timing is unknowable.  A caller that needs as-of
    discipline must use the dated rows only.
    """
    project_of = dict(
        zip(tickets["source_issue_id"].astype(str), tickets["project_key"].astype(str),
            strict=True)
    )
    records: list[dict[str, Any]] = []
    if not change_rows.empty:
        work = change_rows.copy()
        work["field_name"] = (
            work["field_name"].fillna("").astype(str).str.strip().str.casefold()
        )
        work = work[work["field_name"].eq("sprint")]
        work["changed_at"] = pd.to_datetime(work["changed_at"], errors="coerce")
        work = work.sort_values(
            ["issue_id", "changed_at", "change_id"], kind="stable", na_position="last"
        )
        for row in work.itertuples(index=False):
            issue_id = _stable_id(row.issue_id)
            if issue_id is None or issue_id not in project_of:
                continue
            joined = _sprint_id_set(row.to_value) - _sprint_id_set(row.from_value)
            for sprint_jira_id in sorted(joined):
                records.append(
                    {
                        "source_issue_id": issue_id,
                        "project_key": project_of[issue_id],
                        "sprint_jira_id": int(sprint_jira_id),
                        "added_at": (
                            None if _is_missing(row.changed_at)
                            else pd.Timestamp(row.changed_at)
                        ),
                        "provenance": (
                            "change_log_sprint_join"
                            if not _is_missing(row.changed_at)
                            else "omitted_undated_sprint_change_log"
                        ),
                    }
                )

    jira_id_by_sprint_id = {
        _stable_id(row.sprint_id): row.sprint_jira_id
    ...
    for row in tickets.itertuples(index=False):
        ...
        if sprint_id is None:
            continue
        sprint_jira_id = jira_id_by_sprint_id.get(sprint_id)
        if sprint_jira_id is None or _is_missing(sprint_jira_id):
            continue
        records.append(
            {
                ...
                "sprint_jira_id": int(sprint_jira_id),
                "added_at": None,
                "provenance": "final_snapshot_join_time_unknown",
            }
        )

    frame = pd.DataFrame.from_records(
        records,
        columns=["source_issue_id", "project_key", "sprint_jira_id", "added_at",
                 "provenance"],
    )
    if frame.empty:
        frame["added_at"] = pd.to_datetime(frame["added_at"], errors="coerce")
        frame["sprint_jira_id"] = frame["sprint_jira_id"].astype("int64")
        return frame
    frame["added_at"] = pd.to_datetime(frame["added_at"], errors="coerce")
    return frame.sort_values(
        ["project_key", "sprint_jira_id", "added_at", "source_issue_id"],
        kind="stable", na_position="last",
    ).reset_index(drop=True)


def _sprints_frame(raw_sprints: pd.DataFrame) -> pd.DataFrame:
    """Normalize the sprint calendar, keeping every recorded boundary explicit."""
    sprints = raw_sprints.copy()
    sprints["sprint_id"] = sprints["sprint_id"].map(_stable_id)
    sprints["project_key"] = sprints["project_key"].astype(str)
    sprints["name"] = sprints["name"].fillna("").astype(str)
    sprints["state"] = sprints["state"].fillna("").astype(str)
    for column in ("start_date", "end_date", "activated_date", "complete_date"):
        sprints[column] = pd.to_datetime(sprints[column], errors="coerce")
    sprints["sprint_jira_id"] = pd.to_numeric(
        sprints["sprint_jira_id"], errors="coerce"
    ).astype("Int64")
    duplicated = sprints.duplicated(subset=["project_key", "sprint_jira_id"], keep=False)
    if bool(duplicated.any()):
        # The change log identifies a sprint by (project, Jira id); a duplicate would
        # make that identification ambiguous, which no downstream rule could repair.
        raise ValueError(
            "sprint Jira ids are not unique within a project: "
            + ", ".join(
                sorted(
                    f"{row.project_key}:{row.sprint_jira_id}"
                    for row in sprints.loc[duplicated].itertuples(index=False)
                )
            )
        )
    return sprints[
        ["sprint_id", "sprint_jira_id", "project_key", "name", "state", "start_date",
         "end_date", "activated_date", "complete_date"]
    ].sort_values(["project_key", "sprint_jira_id"], kind="stable").reset_index(drop=True)
...
def export(
    ...
    raw_sprints = pd.read_sql(SPRINTS_SQL, eng, params=params)
    ...
    sprints = _sprints_frame(raw_sprints)
    membership = _sprint_membership_frame(tickets, change_rows, sprints)
    ...
    _write_parquet(sprints, output_dir / "sprints.parquet")
    _write_parquet(membership, output_dir / "sprint_membership.parquet")

    dated = int(membership["provenance"].eq("change_log_sprint_join").sum())
    print(
        f"Wrote {len(tickets):,} audit-complete tickets and {len(people):,} eligible people "
        f"across {len(projects):,} projects -> {output_dir}"
    )
    print(
        f"Wrote {len(sprints):,} sprints and {len(membership):,} sprint memberships "
        f"({dated:,} dated change-log joins, {len(membership) - dated:,} undated "
        f"final-snapshot rows)"
    )
