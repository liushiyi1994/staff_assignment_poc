...
The PoC deliberately reuses the PRD's ontology (Contribution, Skill, Specialization, projections, provenance), so promotion is additive, not a rewrite: swap TAWOS loader → curator-mediated ingestion (PRD scope) → Jira API/webhooks; swap project-scoped opaque IDs → authorized roster identity resolution; add curator review UI; add availability as a runtime filter from Sanskrit; move incremental updates from delta batches → Graphiti-style temporal upserts.
